...
dnl
dnl SOX_CHECK_HEADERS(HEADERS, [IF-FOUND], [IF-NOT-FOUND], [EXTRA-HEADERS])
dnl
...
AC_DEFUN([SOX_CHECK_HEADERS], [
    sox_ch_found=no
    AC_CHECK_HEADERS([$1], [sox_ch_found=yes; break], [],
        [m4_map([SOX_INCLUDE], m4_split([$4]))])
    AS_CASE([$sox_ch_found], [yes], [$2], [$3])
])
...
dnl
dnl SOX_FMT_HEADERS(NAME, HEADERS, [INCLUDES], [SECTION])
dnl
dnl Wrapper for SOX_FMT with SOX_CHECK_HEADERS as test.
dnl
AC_DEFUN([SOX_FMT_HEADERS], [
    SOX_FMT([$1], [SOX_CHECK_HEADERS([$2], [], [HAVE_[]T=no], [$3])], [$4])
])

dnl
dnl SOX_FMT_LIB(NAME, HEADERS, LIB, FUNC, [INCLUDES], [EXTRA-LIBS], [SECTION])
dnl
